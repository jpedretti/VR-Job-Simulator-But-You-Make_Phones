...
namespace com.NW84P
{
    ...
    public class ActionBasedControllerManager : MonoBehaviour
    {
        ...
        private bool m_shouldTeleport = true;
        private bool m_shouldTeleportCancel = true;
        private bool m_shouldSnapTurn = true;
        private bool m_shouldTurn = true;
        ...
        void UpdateLocomotionActions()
        {
            ...
            SetEnabled(m_Move, m_SmoothMotionEnabled && m_shouldMove);
            SetEnabled(m_TeleportModeActivate, !m_SmoothMotionEnabled && m_shouldTeleport);
            SetEnabled(m_TeleportModeCancel, !m_SmoothMotionEnabled && m_shouldTeleportCancel);

            // Disable ability to turn when using continuous movement
            SetEnabled(m_Turn, !m_SmoothMotionEnabled && m_SmoothTurnEnabled && m_shouldTurn);
            SetEnabled(m_SnapTurn, !m_SmoothMotionEnabled && !m_SmoothTurnEnabled && m_shouldSnapTurn);
        }
